...
def eval_img_save(x, name, k, epoch):
    ...
    if name == 'real_images':
        # 8， 8
        array2raster(os.path.join(real_dir, 'real_images_{}.tif'.format(k + 1)),
...
class cal_gradient(nn.Module):
    def __init__(self):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        super(cal_gradient, self).__init__()
...
def JQM(ps, l_ms, pan, alpha=0.2, beta=0.8):
    qhr = QHR(ps, pan)
    qlr = QLR(ps, l_ms)
