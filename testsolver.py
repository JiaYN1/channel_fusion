...
from model.fuse_v7 import fuse
...
class Testsolver(object):
    def __init__(self):
        self.batch_size = 1
        # self.model1 = net()
        self.model2 = fuse(channels=32, num_of_layers=8)
    ...
    def test_color_net(self):
        count = 0
        # self.model1.eval()
        avg_time = []
        psnr_list, ssim_list, cc_list, sam_list, ergas_list = [], [], [], [], []
        test_record_name = '/test_only_color_record.txt'
        ...
        for batch in self.test_dataloader:
            ...
            time0 = time.time()
            with torch.no_grad():
                _, _, test_fused_images = self.model1(img_pan, img_lr)
            time1 = time.time()
            ...
            self.test_img_save(test_fused_images, 'test_fused_images', self.epoch1, count)
        ...
        test_record.write(
            "Test avg metrics: PSNR: {:.10f}, SSIM: {:.10f}, CC: {:.10f}, SAM: {:.10f}, ERGAS: {:.10f}\n".format(
    ...
    def test_all(self):
        ...
        for batch in self.test_dataloader:
            ...
            metrics = torch.zeros(5)
            with torch.no_grad():
                # test_fused_images = self.model1(img_pan, img_lr)
                lr_2, lr_4, test_fused_images = self.model2(img_pan, img_lr)  # 网络输出
            time1 = time.time()
            print('==>Save the test_fused_images')
            metrics[:] = torch.Tensor(get_metrics(test_fused_images, target))
            batch_psnr.append(metrics[0].cpu())
            ...
            # self.test_img_save(lr_2, 'lr_2', self.epoch2, count)
            # self.test_img_save(lr_4, 'lr_4', self.epoch2, count)
            # real_images = target.cpu()
            # self.test_img_save(real_images, 'real_images', self.epoch1, count)
            # lrms_images = img_lr_u.cpu()
            # self.test_img_save(lrms_images, 'lrms', self.epoch1, count)
            # pan_images = img_pan.cpu()
            # self.test_img_save(pan_images, 'test_pan_images', self.epoch1, count)
            print("===> Save the test_fused_images || Timer: %.4f sec." % (time1 - time0))
            avg_time.append(time1 - time0)
        test_record.write("AVG Timer: %.4f sec." % (np.mean(avg_time)))
        test_record.write(
    ...
    def test_img_save(self, x, name, epoch, count):
        ...
        test_dir = args.output_dir + '/GF2/channel/fuse_v7/'
        real_dir = args.output_dir + '/GF2/channel/real/'
        ...
        if name == 'test_fused_images':
            ...
            array2raster(os.path.join(real_dir, 'real_images_{}.tif'.format(count)),
                         [0, 0], 2.4, 2.4, denorm(x[0].transpose(2, 0, 1)), 4)
        elif name == 'test_pan_images':
            array2raster(os.path.join(real_dir, 'test_pan_images_{}.tif'.format(count)),
                         [0, 0], 2.4, 2.4, denorm(x[0].reshape(x.shape[1], x.shape[2])), 1)
        else:
            array2raster(os.path.join(real_dir, 'test_lrms_images_{}.tif'.format(count)),
                         [0, 0], 2.4, 2.4, denorm(x[0].transpose(2, 0, 1)), 4)
