...
if __name__ == '__main__':
    ...
    # testDir = dataDir + '/Dataset/test_%s' % satellite1
    # evalDir = dataDir + '/Dataset/eval%d' % blk
    # originTestDir = dataDir + '/Dataset/originTestWV2%d' % blk
    ...
    #                 continue
    #
    #             array2raster('%s/%d_mul.tif' % (originTestDir, originCount), rasterOrigin, 2.4, 2.4,
    #                          img_mul[:, y * 4:(y + blk) * 4, x * 4:(x + blk) * 4], 4)
    #             array2raster('%s/%d_lr_u.tif' % (originTestDir, originCount), rasterOrigin, 2.4, 2.4,
    #                          img_lr_u[:, y * 4:(y + blk) * 4, x * 4:(x + blk) * 4], 4)
    #             array2raster('%s/%d_lr.tif' % (originTestDir, originCount), rasterOrigin, 2.4, 2.4,
    #                          img_lr[:, y:(y + blk), x:(x + blk)], 4)
    #             array2raster('%s/%d_pan.tif' % (originTestDir, originCount), rasterOrigin, 2.4, 2.4,
    #                          img_pan[y * 4:(y + blk) * 4, x * 4:(x + blk) * 4], 1)
    #
    #
    #             originCount += 1
